...
const range = 'Sheet1!G46:G82'; // Диапазон данных
...
fetch(url)
  ...
  .then(data => {
    if (data.values && data.values.length > 0) {
      ...
      document.getElementById('b1').textContent = data.values[23]; 
      document.getElementById('b2').textContent = data.values[24]; 
      document.getElementById('b3').textContent = data.values[25]; 
      document.getElementById('b4').textContent = data.values[26]; 
